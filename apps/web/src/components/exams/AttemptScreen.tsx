...
const OptionButton = ({ active, className, children, ...rest }: OptionButtonProps) => (
  <button
    type="button"
    className={cn(
      "w-full rounded-[18px] border border-border/70 bg-surface px-4 py-3 text-left text-sm text-text-primary shadow-elev-sm transition duration-swift ease-fluid hover:border-brand/40 hover:bg-brand-light/40 hover:text-brand",
...
export const AttemptScreen = ({ questions, onSubmit }: AttemptScreenProps) => {
  ...
  if (!question) {
    return (
      <div className="rounded-[24px] border border-dashed border-border bg-surface p-5 text-sm text-text-secondary">
    ...
    <div className="flex flex-col gap-5">
      <header className="flex items-center justify-between gap-3">
        <div className="flex flex-col gap-2">
          ...
          <h2 className="text-base font-semibold leading-snug text-text-primary">{question.text}</h2>
        </div>
        <span className="rounded-full border border-border bg-surface-alt px-3 py-1 text-xs text-text-secondary">
      ...
      <footer className="mt-2 flex items-center justify-between gap-3">
        <button
          ...
          className="rounded-full border border-border bg-surface-alt px-4 py-2 text-xs font-semibold uppercase tracking-wide text-text-secondary transition duration-swift ease-fluid enabled:hover:border-brand/30 enabled:hover:text-text-primary disabled:opacity-50"
        >
          Oldingi
        </button>
